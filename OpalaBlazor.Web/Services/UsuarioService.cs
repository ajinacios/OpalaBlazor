...
namespace OpalaBlazor.Web.Services
{
    public class UsuarioService : IUsuarioService
    {
        ...
        public async Task<IEnumerable<UsuarioDto>> GetListAll()
        {
            try
            {
                var response = await this.httpClient.GetAsync("Usuarios/ListAll");
        ...
        public async Task<UsuarioDto> GetOneId(int id)
        {
            try
            {
                var response = await httpClient.GetAsync($"Usuarios/porid/{id}");
                if (response.IsSuccessStatusCode)
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.NoContent)
                    {
